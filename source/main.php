...
<script src="./js/bootstrap.min.js"></script>
   <link rel="stylesheet" href="./my_css.css?<?php echo date()  ?>">
...
jQuery(document).ready(function() {
	...
	//Slide
			...
			$("#ex6").on("slide", function(slideEvt) {
				$("#totalmessages").text(slideEvt.value);
				console.log('slider');
			});


 
	jQuery( "#users","#contacts").hide();
		
jQuery( "#click_user" ).click(function() {
	jQuery( "#log").hide();
	jQuery("#users").hide();
...
jQuery( "#click_log" ).click(function() {
	jQuery( "#log").hide();
	jQuery("#users").hide();
	jQuery("#contacts").hide();
	jQuery( "#log").show();
...
jQuery( "#click_contacts" ).click(function() {
...
<div class="row show-grid">
                                ...
                                <div class="col-xs-12 col-md-6">
										...
										<div style="_display:none;">
												<input id="ex6" type="text" data-slider-min="5" data-slider-max="300" data-slider-step="5" data-slider-value="5"/>
												<span id="ex6CurrentSliderValLabel">&nbsp;&nbsp;Nombre total de messages à envoyer: <span id="totalmessages">5</span></span>
												<br>	
										</div>
								</div>
...
<div class="panel panel-default">
                        ...
                        <!-- /.panel-heading -->
                        <div class="panel-body log_users">
                            <!-- Nav tabs -->
                            <ul class="nav nav-tabs">
                                <li class="active"><a href="#log" data-toggle="tab" id="click_log" >LOG</a>                      </li>
                                <li><a href="#users" data-toggle="tab"  id="click_user">USERS</a>                                </li>
                                <li><a href="#contacts" data-toggle="tab" id="click_contacts">CONTACTS</a>                      </li>
                            </ul>

                            <!-- Tab panes --> 
                            <div class="tab-content">
                                <div class="tab-panel fade DivWithScroll" id="contacts" style="display: none;">
                                    <h4></h4>
                                    <?php   include './contacts.php' ?>
                                </div>
                                <div class="tab-panel fade in active DivWithScroll" id="log">
                                    <h4>LOG</h4>
										<div id="results">
                                ...
                                <div class="tab-panel fade DivWithScroll" id="users" style="display: none;">
                                    <h4></h4>
...
function ExecuteBot(token){
		...
		data = {token: token , user: user, search: busqueda, message: message , totalmessages: $('#totalmessages').text()  } ;
...
if ($('#fin').text() == 'NO' || first    ) {
		 $.ajax({
		  ...
		  data: {"user": user}
			...
			$('#log').html('<div id="spinner"  style="width: 100%;" > <i class="fa fa-refresh fa-spin" style="font-size:24px"></i>	</div><h2>Log</h2>'+data);
